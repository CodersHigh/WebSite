	...
	$.retinate = {
		...
		process_videos: function(images) {
			$(images).each(function() {
				...
				if(!is_retina && $.inArray(filetype, $.retinate.image_types) > -1) {
					retina = original.substr(0, dot) + $.retinate.retina_tag + '.' + filetype;
					$(this).attr('poster', retina).attr('data-' + $.retinate.data_tag + '-status', $.retinate.status_replaced);
				}
			});
		},
		...
		process_css: function(images, property) {
			$(images).each(function() {
				...
				if(!is_retina && $.inArray(filetype, $.retinate.image_types) > -1) {
					retina = original.substr(0, dot) + $.retinate.retina_tag + '.' + filetype;
					$(this).css(property, 'url("' + retina + '")').attr('data-' + $.retinate.data_tag + '-status', $.retinate.status_replaced);
				}
			});
		}
	};
